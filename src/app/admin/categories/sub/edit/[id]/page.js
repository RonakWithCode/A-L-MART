...
export default function EditSubCategory({ params }) {
    ...
    const fetchSubCategory = async () => {
        try {
            const subCategory = await categoryService.getSubCategory(params.id);
            setFormData({
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (imageFile) {
                ...
                if (formData.image) {
                    try {
                        await categoryService.deleteSubCategoryImage(formData.image);
                    } catch (error) {
            ...
            await categoryService.updateSubCategory(params.id, subCategoryData);
            toast.success('Sub-category updated successfully');
            router.push('/admin/categories/sub');

        } catch (error) {
            console.error('Update Error:', error);
            setError(error.message || 'Failed to update sub-category');
        ...
        <div className="max-w-2xl mx-auto pb-20">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button
                    ...
                    disabled={saving}
                >
                    {saving ? 'Updating...' : 'Update Sub-Category'}
                </button>
            </form>
        </div>
    );
